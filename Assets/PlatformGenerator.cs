...
public class PlatformGenerator : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        if (_waitTimeRemaining < 0f)
        {
            ...
            var platformObjectIndex = Random.Range(0, PlatformPrefabs.Count);
            var platformOffsetIndex = Random.Range(0, SpawnableYOffsets.Count);
            var platformWidth = Random.Range(effectiveMinPlatformWidth, effectiveMaxPlatformWidth);
            var platformObject = PlatformPrefabs[platformObjectIndex];
            var platformYOffset = SpawnableYOffsets[platformOffsetIndex];
